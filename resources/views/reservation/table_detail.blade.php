    ...
    x-data="reservationHandler(
        ...
        <div class="px-8 mt-6">
            ...
            <div class="flex flex-col gap-6 text-gray-300 text-lg">
                <div class="flex items-center gap-3"><img src="{{ asset('img/wifi.png') }}" class="w-6 h-6"> WiFi</div>
                <div class="flex items-center gap-3"><img src="{{ asset('img/cafe.png') }}" class="w-6 h-6"> Cafe</div>
                <div class="flex items-center gap-3"><img src="{{ asset('img/toilet.png') }}" class="w-6 h-6"> Toilet</div>
                <div class="flex items-center gap-3"><img src="{{ asset('img/parking.png') }}" class="w-6 h-6"> Parkiran</div>
            </div>
        </div>

        {{-- SLIDER TANGGAL --}}
        <div x-data="{ scroll: 0 }" class="relative w-full mt-10">
            <button @click="scroll -= 300; $refs.daySlider.scrollLeft = scroll" class="absolute left-3 top-1/2 -translate-y-1/2 z-20">
                <img src="{{ asset('img/tombol-kiri.png') }}" class="w-10 h-10 opacity-80 hover:opacity-100 transition">
            </button>

            <div x-ref="daySlider" class="flex gap-6 overflow-x-auto no-scrollbar px-14 scroll-smooth">
                @php 
                    $startDate = \Carbon\Carbon::now('Asia/Jakarta'); // Mulai dari hari ini (WIB)
                    // Jika jam < 02:00, anggap masih hari kemarin secara operasional
                    if ($startDate->hour < 2) {
                        $startDate->subDay();
                    }
                @endphp
            ...
            <button @click="scroll += 300; $refs.daySlider.scrollLeft = scroll" class="absolute right-3 top-1/2 -translate-y-1/2 z-20">
                <img src="{{ asset('img/tombol-kanan.png') }}" class="w-10 h-10 opacity-80 hover:opacity-100 transition">
            </button>
        </div>
        <style>.no-scrollbar::-webkit-scrollbar { display: none; }</style>

        {{-- SLIDER JADWAL (HTML Structure sama) --}}
            ...
            <div x-show="selectedDate" x-transition:enter="transition ease-out duration-300"
                ...
                <div class="flex items-center justify-between mb-6">
                    ...
                    <div class="bg-green-500/20 border border-green-500/30 rounded-xl px-5 py-2.5 backdrop-blur-sm">
                        <span class="text-green-400 font-bold text-lg" x-text="countAvailable"></span>
                        <span class="text-green-300 ml-1">Slot Tersedia</span>
                    </div>
                </div>

                <div x-data="{ scroll: 0 }" class="relative w-full px-6">
                    <button @click="scroll -= 300; $refs.timeSlider.scrollLeft = scroll" class="absolute left-2 top-1/2 -translate-y-1/2 z-20 bg-white/10 backdrop-blur-md rounded-full p-3 hover:bg-white/20 transition-all duration-300 shadow-lg border border-white/20">
                    ...
                    <div x-ref="timeSlider" class="flex gap-4 overflow-x-auto no-scrollbar scroll-smooth px-14 mb-10">
                        <template x-for="slot in availableSlots" :key="slot.time">
                            <button @click="toggleSlot(slot)" 
                                ...
                                :class="{
                                    'bg-red-900/20 opacity-50 cursor-not-allowed border-red-900/30': slot.isBooked,
                                    'bg-gray-800 opacity-30 cursor-not-allowed border-gray-800': slot.isPast && !slot.isBooked,
                                    ...
                                    'bg-white/5 text-[#F4EFE7] hover:bg-white/10 hover:scale-105 border-white/20': !slot.isBooked && !slot.isPast && !slot.isSelected
                                }"
                                class="min-w-[170px] rounded-2xl border backdrop-blur-sm flex flex-col items-center justify-center p-5 transition-all duration-300 relative overflow-hidden">
                                ...
                                <template x-if="slot.isBooked">
                                    <span class="text-xs text-red-400 mt-2 bg-red-500/10 px-3 py-1 rounded-full border border-red-500/20">Terisi</span>
                                </template>
                                <template x-if="slot.isPast && !slot.isBooked">
                                    <span class="text-xs text-gray-400 mt-2 bg-gray-700/50 px-3 py-1 rounded-full">Lewat</span>
                                </template>
                            </button>
                        </template>
                    </div>

                    <button @click="scroll += 300; $refs.timeSlider.scrollLeft = scroll" class="absolute right-2 top-1/2 -translate-y-1/2 z-20 bg-white/10 backdrop-blur-md rounded-full p-3 hover:bg-white/20 transition-all duration-300 shadow-lg border border-white/20">
        ...
        {{-- BOTTOM BAR --}}
        <div x-show="selectedSlots.length > 0" class="fixed bottom-0 left-0 right-0 bg-[#181C14] border-t-4 border-[#3C3D37] backdrop-blur-xl px-8 py-6 flex items-center justify-between z-30 shadow-2xl">
            <div>
                <p class="text-[#FFF4E4] font-semibold text-xl">Total Reservasi</p>
                <p class="font-bold text-white text-2xl" x-text="totalPriceFormatted"></p>
            </div>
            <form method="POST" action="{{ route('customer.reservation.cart') }}">
                ...
                <button type="submit" class="bg-[#C1121F] text-[#FFF4E4] hover:bg-[#A00F1B] text-xs font-bold py-4 px-10 rounded-xl transition shadow-lg hover:scale-105 duration-300 shadow-red-900/50 flex items-center gap-3 cursor-pointer">
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('reservationHandler', (tableId, priceSiang, priceSore, priceMalam, bookedSlotsJS, serverDateStr, serverHour, hasUnpaid, unpaidTransactionId) => ({
            ...
            generateAvailableSlots() {
                ...
                // --- LOGIKA SEDERHANA URUTAN 0 - 23 ---
                // Tidak ada lagi logika "Next Day" atau "Business Day" yang rumit.
                // 00:00 adalah jam 0 hari ini. 23:00 adalah jam 23 hari ini.
                ...
                for (let i = 0; i < 24; i++) {
                    ...
                    // Logic Harga (Sederhana berdasarkan jam murni)
                    let currentPrice;

<x-Layout>
    ...
    <section class="min-h-screen bg-[#0e0f0b] text-[#F4EFE7] font-poppins pb-32" x-data="reservationHandler(
        {{ $table->table_id }},
        {{ $hargaSiang }},
        {{ $hargaSore }},
        {{ $hargaMalam }},
            ...
            <div class="absolute bottom-0 left-0 w-full p-6 md:p-10 z-10">
                <div class="container mx-auto max-w-5xl">
                    ...
                    <div class="flex items-center gap-2 text-sm md:text-base text-gray-300">
                        <span class="bg-[#e9d9c9] text-black px-2 py-0.5 rounded text-xs font-bold uppercase">Meja
                            Biliar</span>
                        <span>•</span>
        ...
        <div class="container mx-auto max-w-5xl px-6 md:px-10 mt-8 space-y-12">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="flex items-center gap-3 bg-white/5 p-3 rounded-lg border border-white/5">
                            <img src="{{ asset('img/wifi.png') }}" class="w-5 h-5 opacity-70"> <span
                                class="text-sm text-gray-300">WiFi</span>
                        </div>
                        <div class="flex items-center gap-3 bg-white/5 p-3 rounded-lg border border-white/5">
                            <img src="{{ asset('img/cafe.png') }}" class="w-5 h-5 opacity-70"> <span
                                class="text-sm text-gray-300">Cafe</span>
                        </div>
                        <div class="flex items-center gap-3 bg-white/5 p-3 rounded-lg border border-white/5">
                            <img src="{{ asset('img/toilet.png') }}" class="w-5 h-5 opacity-70"> <span
                                class="text-sm text-gray-300">Toilet</span>
                        </div>
                        <div class="flex items-center gap-3 bg-white/5 p-3 rounded-lg border border-white/5">
                            <img src="{{ asset('img/parking.png') }}" class="w-5 h-5 opacity-70"> <span
                                class="text-sm text-gray-300">Parkir</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex gap-2">
                        <button @click="$refs.daySlider.scrollBy({left: -200, behavior: 'smooth'})"
                            class="p-2 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 transition">
                ...
                <div x-ref="daySlider" class="flex gap-4 overflow-x-auto no-scrollbar pb-4 snap-x">
                    ...
                    @for ($i = 0; $i < 7; $i++)
                        ...
                        <button @click="selectDate('{{ $dateString }}')"
                            ...
                            <span
                                class="text-xs font-medium uppercase tracking-wider opacity-80">{{ $day->translatedFormat('l') }}</span>
            ...
            <div x-show="selectedDate" x-transition.opacity class="space-y-6">
                <div class="flex items-center justify-between border-b border-white/10 pb-4">
                    ...
                    <div
                        class="flex items-center gap-2 text-sm text-green-400 bg-green-900/20 px-3 py-1 rounded-full border border-green-900/30">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                    <template x-for="slot in availableSlots" :key="slot.time">
                        <button @click="toggleSlot(slot)" :disabled="slot.isBooked || slot.isPast"
                            :class="{
                                'bg-red-900/20 border-red-900/30 opacity-60 cursor-not-allowed': slot.isBooked,
                                'bg-gray-800/40 border-gray-700/30 opacity-40 cursor-not-allowed': slot.isPast && !slot
                                    .isBooked,
                                'bg-[#e9d9c9] text-black border-[#e9d9c9] shadow-[0_0_10px_rgba(233,217,201,0.5)] transform scale-[1.02]': slot
                                    .isSelected,
                                'bg-white/5 border-white/10 text-gray-300 hover:bg-white/10 hover:border-white/30': !
                                    slot.isBooked && !slot.isPast && !slot.isSelected
                            }"
            ...
            <template x-if="!selectedDate">
                <div
                    class="flex flex-col items-center justify-center py-16 border-2 border-dashed border-white/10 rounded-2xl bg-white/5">
        ...
        <div x-show="selectedSlots.length > 0" x-transition:enter="transition ease-out duration-300"
            ...
            <div
                ...
                <div class="flex flex-col pl-2">
                    <span
                        class="text-[10px] text-gray-400 font-medium uppercase tracking-wider leading-none mb-0.5">Total
                        Estimasi</span>
                    <div class="flex items-baseline gap-1">
                        ...
                        <span class="text-white font-bebas text-xl md:text-2xl tracking-wide"
                            x-text="totalPrice.toLocaleString('id-ID')"></span>
                    </div>
                </div>
                ...
                <form method="POST" action="{{ route('customer.reservation.cart') }}">
                    ...
                    <button type="submit"
                        ...
                        <svg class="w-3.5 h-3.5 transform transition-transform group-hover:translate-x-1"
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('reservationHandler', (tableId, priceSiang, priceSore, priceMalam, bookedSlotsJS,
                serverDateStr, serverHour, hasUnpaid, unpaidTransactionId) => ({
                tableId: tableId,
                ...
                get countAvailable() {
                    return this.availableSlots.filter(slot => !slot.isBooked && !slot.isPast)
                    .length;
                },
                ...
                generateAvailableSlots() {
                    ...
                    console.log('🔍 Checking Date:', dateSelectedStr);
                    console.log('📅 Booked Slots:', bookedToday);
                    ...
                    for (let i = 0; i < 24; i++) {
                        ...
                        // ✅ FIX: Cek apakah slot ini termasuk dalam bookedToday
                        // bookedToday berisi array waktu mulai dari booking yang ada
                        let isBooked = bookedToday.some(bookedTime => {
                            // Hilangkan detik jika ada format berbeda
                            const cleanBookedTime = bookedTime.substring(0,
                            5); // "14:00" dari "14:00:00"
                            const cleanStartTime = startTimeDB.substring(0,
                            5); // "14:00" dari "14:00:00"

                            console.log(
                                `Comparing: ${cleanStartTime} === ${cleanBookedTime} ? ${cleanStartTime === cleanBookedTime}`
                                );

                            return cleanStartTime === cleanBookedTime;
                        });
                        ...
                        // Debug log
                        if (isBooked) {
                            console.log(`🚫 Slot ${timeString} is BOOKED`);
                        }
                    }

<x-Layout>
    ...
    <section class="min-h-screen bg-[#0e0f0b] text-white px-4 py-20 flex flex-col items-center">
        ...
        <div class="w-full max-w-4xl space-y-8">
            ...
            <div class="w-full border px-6 py-4 rounded-xl flex flex-col md:flex-row justify-between items-center gap-4 backdrop-blur-sm {{ $statusColors }}">
                            ...
                            @if ($reservationData)
                                <div class="flex justify-between items-start">
                                    <div class="flex gap-4">
                                        <div class="w-12 h-12 bg-[#e9d9c9]/10 rounded-lg flex items-center justify-center border border-[#e9d9c9]/20">
                                            {{-- Ikon Meja --}}
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-[#e9d9c9]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                                            </svg>
                                        </div>
                                        <div>
                                            <p class="font-bold text-lg">{{ $reservationData->table->nama ?? 'Meja Terhapus' }}</p>
                                            <div class="text-sm text-gray-400 mt-1 space-y-1">
                                                <p>Tanggal: {{ \Carbon\Carbon::parse($reservationData->tanggal_reservasi)->format('d M Y') }}</p>
                                                <p>Jam: {{ \Carbon\Carbon::parse($reservationData->waktu_mulai)->format('H:i') }} - {{ \Carbon\Carbon::parse($reservationData->waktu_selesai)->format('H:i') }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="flex gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-400 mt-1">
                                                {{ $item->quantity }} x Rp{{ number_format($item->harga / $item->quantity, 0, ',', '.') }}
                                            </p>
                                        </div>
                                    </div>
                                    <p class="font-mono font-medium">Rp{{ number_format($item->harga, 0, ',', '.') }}</p>
                                </div>
                            @endforeach
                        </div>
                        ...
                        <div class="space-y-4 text-sm">
                            ...
                            <div>
                                ...
                                <p class="text-white">
                                    ...
                                    {{ $transaction->created_at ? $transaction->created_at->timezone('Asia/Jakarta')->format('H:i') : '-' }} WIB
                                </p>
                            </div>

                    ...
                    <div x-data="{ open: false }" class="relative">
                        <button @click="open = !open" class="flex items-center focus:outline-none relative">
                            <img src="{{ asset('img/profilsaya.png') }}" alt="User"
                                class="w-10 h-10 rounded-full border-2 border-transparent hover:border-[#F4EFE7] transition">

                            @if ($hasUnpaid)
                                <span
                                    class="absolute top-0 right-0 block h-3 w-3 rounded-full ring-2 ring-black bg-red-500 animate-pulse"></span>
                            @endif
                        </button>

                        {{-- 'x-cloak' agar tidak kedip saat reload. Pastikan di CSS global [x-cloak] { display: none !important; } --}}
                        <div x-show="open" x-cloak @click.away="open = false" x-transition
                            ...
                            <a href="{{ route('customer.transaction.history') }}"
                                class="flex justify-between items-center px-4 py-2 text-gray-700 hover:bg-gray-200 transition">
        ...
        @else
            <nav class="flex items-center space-x-2">
                <a href="{{ route('register') }}"
                    class="bg-gray-600 hover:bg-gray-500 text-[#F4EFE7] font-medium text-sm py-2 px-4 rounded-full transition duration-150 ease-in-out">
                    Daftar
                </a>
                <a href="{{ route('login') }}"
                    class="bg-[#F4EFE7] hover:bg-white text-black font-medium text-sm py-2 px-4 rounded-full transition duration-150 ease-in-out">
                    Masuk
                </a>
            </nav>
        @endif
